                                       ...
                                       dispatch_async(dispatch_get_main_queue(), ^(void){
                                           if (jsonData.count) {
                                               ...
                                               if ([jsonData firstObject][quality]) {
                                                   ...
                                                   [self setObjectToUserDefaults:dictToSave];
                                                   NSURL *url = [NSURL URLWithString:jsonData[0][quality]];
                                                   [SDWebImageManager.sharedManager loadImageWithURL: url options: SDWebImageContinueInBackground progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {

                                                   } completed:^(UIImage * _Nullable image, NSData * _Nullable data, NSError * _Nullable error, SDImageCacheType cacheType, BOOL finished, NSURL * _Nullable imageURL) {
                                                       [SDImageCache.sharedImageCache removeImageForKey: jsonData[0][quality] withCompletion:^{
                                                           [SDImageCache.sharedImageCache storeImage: image forKey: vimdeoURLString completion:^{
